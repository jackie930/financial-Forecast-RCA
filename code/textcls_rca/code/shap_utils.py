...
def rank_tokens_by_values(shap_values, grouping_threshold=0.1):
    all_tokens = []
    all_values = []
    for i, v in enumerate(shap_values[:1]):
        values, clustering = unpack_shap_explanation_contents(v)
        tokens, values, group_sizes = process_shap_values(v.data, values, grouping_threshold, '', clustering)
        all_tokens += list(tokens)
        all_values += list(values)
    value_index = np.argsort(all_values)
    sorted_values = np.array(all_values)[value_index]
    sorted_tokens = list(np.array(all_tokens)[value_index])
#     print('tokens number: ', len(sorted_tokens))
#     print('top-20 tokens: ', sorted_tokens[::-1][:20])
#     print('top-20 values: ', sorted_values[::-1][:20])
    data_dict = {'tokens': sorted_tokens[::-1], 'shaply-value': sorted_values[::-1]}
    score_df = pd.DataFrame(data_dict)
    return score_df


def show_top_n_tokens(shap_values, top_n=10, grouping_threshold=0.1):
    score_df = rank_tokens_by_values(shap_values, grouping_threshold=grouping_threshold)
    sns.barplot(data=score_df[:top_n], x="shaply-value", y="tokens",color='c')
    plt.ylabel('')
    plt.title('top %s tokens'%(top_n), fontsize=12)
    plt.show()
